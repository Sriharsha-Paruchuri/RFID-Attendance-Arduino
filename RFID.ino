...
byte cards[numOfCards][5] = {{0xf3, 0x07, 0x00, 0x0d},{0xe3, 0xcd, 0xc8, 0x16},{0xD9, 0x45, 0xE5, 0x6E},{0xA9, 0x76, 0x47, 0xB8},{0x59, 0x3F, 0x16, 0x98}}; // array of UIDs of rfid cards
...
String names[numOfCards] = {"Sriharsha P","Manoj B ","Manoj Akalanka","Milan Sankalpa","Chamila Bandara"};//student names
long sNumbers[numOfCards] = {07,10,16389,16322,16323};//student sNumbers
...
//  setup for lcd screen
  ...
  rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));//time will set auto when it compiles
   //rtc.adjust(DateTime(2021, 12, 18, 20, 41, 30));// manually set time
  }
  if (rtc.lostPower()) {
  ...
  Serial.println(F("\t\t\t<<<< ECE_D Attendance >>>>\n")); // introduction
  Serial.println(F("COUNT\tRegNO\tNAME\t\tDATE\t\tENTERED\t\tEXIT"));// make four columns
//setup sd card
 ...
 File  myFile = SD.open("ECE_D.txt", FILE_WRITE);
  if (myFile) {
    myFile.print(F("  \t**** ECE_D Attendance ****\n"));
    myFile.println(F("COUNT\tRegNO\tNAME\t\tDATE\t\tENTERED\t\tEXIT"));
    // close the file:
    myFile.close();
    Serial.println(F("done."));
  } else {
    // Serial.println(F("error opening ECE_D.txt"));
  }

  myFile = SD.open("ECE_D.txt");
  if (myFile) {
    Serial.println(F("ECE_D.txt:"));
...
void servo(){//method for servo motor
  int pos = 0;    // variable to store the servo position
   for (pos = 180; pos >= 0; pos -= 1) { // goes from 0 degrees to 90 degrees
    // in steps of 1 degree
    myservo.write(pos);              
    delay(20);                       
  }
  delay(5000);
   for (pos = 0; pos <= 180; pos += 1) {// goes from 90 degrees to 0 degrees
    myservo.write(pos);             
    delay(20);                       
  }
}
...
void logCardData(String name, long sNumber, int j){
  ...
  digitalWrite(CS_SD,LOW);
  File  myFile = SD.open("ECE_D.txt", FILE_WRITE);//record the data to the sd card
  if (myFile) {
